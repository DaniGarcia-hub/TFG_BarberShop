<x-app-layout>
    <main class="bg-[#262929] pt-[1rem] md:pt-[2rem] pb-[6rem] px-[4vw] max-w-[1400px] mx-auto w-full">
                ...
                <div class="flex gap-[0.5rem] items-center px-[12px] py-[0.2rem] rounded-full bg-[#101111] w-max">
                    <figure class="flex items-center justify-center text-[#D1C7B7]">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="16" height="16">
                            <path fill="rgb(209, 199, 183)" d="M128 0c17.7 0 32 14.3 32 32V64H288V32c0-17.7 14.3-32 32-32s32 14.3 32 32V64h48c26.5 0 48 21.5 48 48v48H0V112C0 85.5 21.5 64 48 64H96V32c0-17.7 14.3-32 32-32zM0 192H448v272c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V192zm305.7 142.7c-6.2-6.2-16.4-6.2-22.6 0L224 393.4l-31.1-31.1c-6.2-6.2-16.4-6.2-22.6 0s-6.2 16.4 0 22.6l42.4 42.4c6.2 6.2 16.4 6.2 22.6 0l112-112c6.2-6.2 6.2-16.4 0-22.6z"/>
                        </svg>
                    </figure>
                    <p class="font-bebas text-[1rem] uppercase tracking-wider text-[#E5E5E5] px-[0.25rem]">
                        Modificando Mi Reserva: #{{ $booking->id }}
                    </p>
                </div>
            </div>

            <a href="{{ route('dashboard') }}" class="w-full sm:w-auto text-center flex items-center justify-center gap-[0.5rem] text-[#413B36] bg-[#D1C7B7] py-[0.5rem] px-[1.5rem] rounded-[0.4rem] border border-[#927860] uppercase no-underline font-bebas text-[1.2rem] shadow-md hover:bg-[#EBE6E1] transition-all duration-300">
        ...
        {{-- Bloque de errores de Laravel: Aquí saltará si la combinación de fecha/hora está ocupada --}}
        @if ($errors->any())
        ...
        <div class="bg-[#EBE6E1] rounded-[8px] p-[1.5rem] md:p-[2.5rem] shadow-xl w-full border border-[#413B36]/20 mb-[1.5rem]">
            ...
            <form method="POST" action="{{ route('clientbookings.update', $booking->id) }}" class="font-poppins text-[#413B36]">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-[1.5rem] mb-[2.5rem]">
                    ...
                    <div class="flex flex-col gap-[0.4rem]">
                        <label class="font-semibold text-[0.9rem]">Cliente</label>
                    ...
                    <div class="flex flex-col gap-[0.4rem]">
                        <label class="font-semibold text-[0.9rem]">Peluquero <span class="text-rose-700">*</span></label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-[1.5rem] mb-[1.5rem]">
                    ...
                    <div class="grid grid-cols-1 gap-[1.5rem] md:col-span-2 mb-[1rem]">
                        <div class="flex items-center gap-[0.75rem] bg-rose-950/5 border border-rose-900/20 rounded-[6px] p-[1rem] w-full">
                            <div class="flex items-center h-5">
                                <input type="checkbox" name="cancel_booking" id="cancel_booking" value="1" {{ old('cancel_booking', $booking->state === 'cancelado' ? '1' : '') == '1' ? 'checked' : '' }}
                                    class="w-4 h-4 text-rose-800 bg-[#FAFAFA] border-[#413B36]/30 rounded focus:ring-rose-800 cursor-pointer">
                            </div>
                            <div class="font-poppins select-none">
                                <label for="cancel_booking" class="font-semibold text-[0.95rem] text-rose-900 cursor-pointer">
                                    Cancelar Reserva
                                </label>
                                <p class="text-stone-500 text-[0.8rem] mt-[0.1rem]">
                                    Si marcas esta casilla, la reserva pasará automáticamente al estado de <span class="font-semibold text-rose-800">Cancelada</span> al guardar.
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                ...
                fetch(url) // Llamada a la función que me cargará las horas.
                    ...
                    .then(result => {
                        ...
                        result.data.forEach(slot => { // por cada horario disponible..
                            ...
                            if (slot.id == "{{ $booking->schedule_id ?? '' }}") option.selected = true;
            ...
            if (barber.value && dateInput.value) cargarHoras(); // Como el editar ya tiene valores, se cargan automáticamente las horas.
        });
