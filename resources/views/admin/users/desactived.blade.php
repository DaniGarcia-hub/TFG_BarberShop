...
@section('content')
    <main class="bg-[#262929] pt-[1rem] md:pt-[2rem] pb-[6rem] px-[1vw]">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-[#927860] pb-[1.5rem] mb-[2.5rem] gap-[1rem] w-full">
            <div>
                <div class="flex gap-[0.5rem] items-center px-[12px] py-[0.2rem] rounded-full bg-[#101111] w-max">
                    ...
                    <p class="font-bebas text-[1rem] uppercase tracking-wider text-rose-300 px-[0.25rem]">Cuentas Suspendidas</p>
                </div>
            </div>

            <a href="{{ route('admin.users.index') }}" class="w-full sm:w-auto text-center flex items-center justify-center gap-[0.5rem] text-[#E5E5E5] bg-[#413B36] py-[0.5rem] px-[1.5rem] rounded-[0.4rem] border border-[#413B36] uppercase no-underline font-bebas text-[1.2rem] shadow-md hover:bg-[#101111] transition-all duration-300">
        ...
        <div class="bg-[#EBE6E1] rounded-[8px] p-[1.25rem] shadow-xl w-full overflow-x-auto scrolling-touch border border-[#413B36]/20 text-[#413B36]">
            @if ($users->count() > 0)
            <table id="users_deleted_table" class="w-full text-left border-collapse min-w-[1000px] font-poppins text-[0.9rem] text-[#413B36]">
                <thead>
                    <tr class="border-b-2 border-[#413B36] text-[#413B36] font-bebas text-[1.2rem] uppercase tracking-wider">
                        ...
                        <th class="pb-[1rem] text-center w-[8%]">Restaurar</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-[rgba(65,59,54,0.15)]">
                    @forelse ($users as $user)
                        <tr class="hover:bg-[rgba(209,199,183,0.25)] transition-colors duration-150 opacity-85">
                            <td class="py-[1rem] px-[0.5rem] font-bold text-[#927860]">
                                #{{ $user->id }}
                            </td>
                            
                            <td class="py-[1rem] font-medium text-[#413B36] decoration-stone-400">
                                {{ $user->name }}
                            </td>
                            ...
                            <td class="py-[1rem]">
                                @if($user->getRoleNames()->isNotEmpty())
                                    <span class="bg-stone-500 text-white text-[0.75rem] font-semibold px-[0.6rem] py-[0.15rem] rounded-[4px] uppercase tracking-wider font-poppins">
                                        {{ $user->getRoleNames()->first() }}
                                    </span>
                                @else
                                    <span class="bg-[#413B36]/10 text-[#413B36]/50 text-[0.75rem] font-medium px-[0.6rem] py-[0.15rem] rounded-[4px] uppercase font-poppins">
                                        Cliente
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="py-[1rem] text-center">
                                <div class="flex items-center justify-center gap-[0.5rem]">
                                    <form action="{{ route('admin.users.restore', $user->id) }}" method="POST" class="inline" onsubmit="return confirm('¿Deseas restaurar la cuenta de este usuario para que vuelva a estar operativo?');">
                                        @csrf
                                        @method('PATCH')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="py-[3rem] text-center font-medium text-[#413B36]/50 italic">
                                No hay ninguna cuenta de usuario desactivada en el archivo.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            @else
                <h2>No se encontraron usuarios desactivados.</h2>
            @endif
