...
@section('content')
    <main class="bg-[#262929] pt-[1rem] md:pt-[2rem] pb-[6rem] px-[1vw]">
            ...
            <div>
                <div class="flex gap-[0.5rem] items-center px-[12px] py-[0.2rem] rounded-full bg-[#101111] w-max">
                    ...
                    <p class="font-bebas text-[1rem] uppercase tracking-wider text-[#E5E5E5] px-[0.25rem]">
                        Modificando Servicio: {{ $haircut->name }} (#{{ $haircut->id }})
                    </p>
                </div>
            </div>
        ...
        <div class="bg-[#EBE6E1] rounded-[8px] p-[1.5rem] md:p-[2.5rem] shadow-xl w-full border border-[#413B36]/20">
            ...
            <form method="POST" action="{{ route('admin.haircuts.update', $haircut->id) }}" enctype="multipart/form-data" class="font-poppins text-[#413B36]">
                @csrf
                @method('PUT')

                <h3 class="font-bebas text-[1.4rem] uppercase tracking-wider text-[#927860] border-b border-[#413B36]/10 pb-[0.3rem] mb-[1.5rem]">
                    Propiedades del Corte
                </h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-[1.5rem] mb-[1.5rem]">
                    ...
                    <div class="flex flex-col gap-[0.4rem] md:col-span-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $haircut->name) }}" required 
                    ...
                    <div class="flex flex-col gap-[0.4rem]">
                        ...
                        <input type="number" name="price" id="price" step="0.01" min="0" value="{{ old('price', $haircut->price) }}" required 
                            class="bg-[#FAFAFA] border border-[#413B36]/30 rounded-[6px] px-[0.8rem] py-[0.6rem] outline-none focus:border-[#927860] focus:ring-1 focus:ring-[#927860] transition-all">
                    </div>

                    <div class="flex flex-col gap-[0.4rem]">
                        <label for="duration" class="font-semibold text-[0.9rem]">Duración Estimada (Minutos) <span class="text-rose-700">*</span></label>
                        <input type="number" name="duration" id="duration" min="1" value="{{ old('duration', $haircut->duration) }}" required 
                            class="bg-[#FAFAFA] border border-[#413B36]/30 rounded-[6px] px-[0.8rem] py-[0.6rem] outline-none focus:border-[#927860] focus:ring-1 focus:ring-[#927860] transition-all">
                    </div>

                    <div class="flex flex-col gap-[0.4rem] md:col-span-2">
                        <label for="description" class="font-semibold text-[0.9rem]">Descripción</label>
                        <textarea name="description" id="description" rows="4" class="bg-[#FAFAFA] border border-[#413B36]/30 rounded-[6px] px-[0.8rem] py-[0.6rem] outline-none focus:border-[#927860] focus:ring-1 focus:ring-[#927860] transition-all resize-none">{{ old('description', $haircut->description) }}</textarea>
                    </div>
                ...
                <div class="grid grid-cols-1 gap-[1.5rem] mb-[2.5rem]">
                    <div class="flex items-center gap-[0.75rem] bg-[#101111]/5 border border-[#413B36]/10 rounded-[6px] p-[1rem] w-full md:w-max">
                        <div class="flex items-center h-5">
                            <input type="checkbox" name="active" id="active" value="1" {{ old('active', !$haircut->trashed() ? '1' : '') == '1' ? 'checked' : '' }}
                        ...
                        <div class="font-poppins select-none">
                            ...
                            <p class="text-stone-500 text-[0.8rem] mt-[0.1rem]">
                                Si se desmarca, este corte de pelo dejará de estar disponible para las reservas.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            @if($haircut->trashed())
                <form action="{{ route('admin.haircuts.restore', $haircut->id) }}" method="POST" class="w-full sm:w-auto inline">
                    @csrf
                    @method('PATCH')
            ...
            @else
                <form action="{{ route('admin.haircuts.destroy', $haircut->id) }}" method="POST" class="w-full sm:w-auto inline" onsubmit="return confirm('¿Estás seguro de que deseas retirar este corte?');">
                    @csrf
                    @method('DELETE')
